...
*Features:* KSU $(parse_bool "$KSU"), SuSFS $(is_true "$SUSFS" && escape_md_v2 "$SUSFS_VERSION" || echo "Disabled"), LXC $(parse_bool "$LXC"), KPM $(parse_bool "$KPM"), BBG $(parse_bool "$BBG"), BBRv3 $(parse_bool "$BBR_V3"), Stock config $(parse_bool "$STOCK_CONFIG")
EOF
    )
